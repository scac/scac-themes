	...
	function churchthemes_top_sermons() {
		...
		$i = 0;
			...
			<?php
				...
				$comments = ($comments == 1) ? __('One Comment', 'churchthemes') : $comments . __(' Comments', 'churchthemes');
				...
				<tr<?php if($i % 2 == 0) echo ' class="alt"'; ?>>
					<td>
						...
						<span class="title" style="clear:none;">
							<a href="post.php?post=<?php esc_attr_e($post->ID) ?>&action=edit"<?php if($comments): ?> title="<?php esc_attr_e($comments) ?>"<?php endif; ?>><?php the_title() ?></a>
						</span>
					</td>
					<td>
					<?php foreach($speakers as $speaker): ?>
						...
						<br>
						<?php endif; ?>
						<a href="edit.php?sermon_speaker=<?php esc_attr_e($speaker->slug) ?>&post_type=ct_sermon" title="<?php esc_attr_e($speaker->count) ?> Sermons by <?php esc_attr_e($speaker->name) ?>"><?php echo $speaker->name ?></a>
					<?php endforeach; ?>
						<br>
					</td>
