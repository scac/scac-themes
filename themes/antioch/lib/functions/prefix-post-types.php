...
function churchthemes_prefix_post_types(){
	...
	foreach ( $post_types as $post_type ) {
		if ( $wpdb->query( $wpdb->prepare( "SELECT * FROM " . $wpdb->posts . " WHERE post_type = %s", $prefix . $post_type ) ) ) {
			return;
		}
		else {
			$wpdb->query( $wpdb->prepare( 'UPDATE ' . $wpdb->posts . ' SET post_type = %s WHERE post_type = %s', $prefix . $post_type, $post_type ) );
			$wpdb->query( $wpdb->prepare( 'UPDATE ' . $wpdb->posts . ' SET guid = replace( guid, "post_type=' . $post_type . '", "post_type=' . $prefix . $post_type . '" )' ) );
		}
	}
}
